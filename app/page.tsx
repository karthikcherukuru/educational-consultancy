...
const testimonials = [
  { name: "Aarav Sharma",   admittedTo: "VIT Chennai",                gender: "male"   as const, quote: "The experts at Heshritha helped me shortlist the right campuses based on my mock scores and handled my entire portfolio. I couldn't have done it without them." },
  { name: "Priya Reddy",    admittedTo: "SRM Institute",              gender: "female" as const, quote: "They didn't just push me into any college; they found the one that perfectly aligned with my goal of studying AI. The career guidance was the game changer." },
  { name: "Karthik Verma",  admittedTo: "Amrita Vishwa Vidyapeetham", gender: "male"   as const, quote: "From profile building to final interview prep, the 15 years of experience really shows. My mentor was available literally 24/7." },
  { name: "Sneha Patel",    admittedTo: "Woxsen University",          gender: "female" as const, quote: "They simplified the complex fee structures, guided us on scholarships, and made the documentation a breeze. My parents and I are so grateful." },
  { name: "Rahul Desai",    admittedTo: "Alliance University",        gender: "male"   as const, quote: "I thought my gap year would ruin my chances. The strategic planning by Heshritha helped me frame my story perfectly. Highly recommended!" },
  { name: "Neha Krishnan",  admittedTo: "Presidency University",      gender: "female" as const, quote: "The best educational consultants in the business. Period. They genuinely care about your future." },
];
...
function AvatarIcon({ gender }: { gender: "male" | "female" }) {
  const colorClass = gender === "female" ? "bg-pink-100 text-pink-600" : "bg-blue-100 text-blue-600";
  return (
    <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center shrink-0 ${colorClass}`}>
      {gender === "female" ? (
        <svg className="w-6 h-6 sm:w-7 sm:h-7" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 2a5 5 0 015 5c0 2.05-1.23 3.81-3 4.58V13h2v2h-2v2h-4v-2H8v-2h2v-1.42C8.23 10.81 7 9.05 7 7a5 5 0 015-5zm0 2a3 3 0 100 6 3 3 0 000-6z" />
        </svg>
      ) : (
        <svg className="w-6 h-6 sm:w-7 sm:h-7" fill="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="flex flex-col w-full min-h-screen bg-white">
        ...
        <div className="relative w-full px-5 sm:px-8 lg:px-12 pt-20 pb-8" style={{ zIndex: 2 }}>
          <div className="max-w-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <a href="tel:+919000850124" className="inline-flex h-14 items-center justify-center gap-2 rounded-2xl bg-white px-8 text-sm font-bold uppercase tracking-wider text-black active:scale-[0.97] transition-transform shadow-xl">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
                Call for Free Advice
              </a>
              <a href="https://wa.me/919000850124" target="_blank" rel="noopener noreferrer" className="inline-flex h-14 items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-8 text-sm font-bold text-white active:scale-[0.97] transition-transform shadow-xl">
      ...
      <section className="bg-white py-6 sm:py-8">
        <div className="flex sm:hidden gap-4 px-5 overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-2" style={{ scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
          {bentoCards.map((card) => (
            <div key={card.title} className="snap-center shrink-0 relative overflow-hidden rounded-3xl flex flex-col justify-end shadow-md" style={{ width: "80vw", height: 260 }}>
        ...
        <div className="hidden sm:grid sm:grid-cols-3 gap-5 container mx-auto px-5 sm:px-6">
          {bentoCards.map((card) => (
            <div key={card.title} className="group relative overflow-hidden rounded-3xl flex flex-col justify-end shadow-md hover:shadow-2xl hover:-translate-y-1 transition-all duration-300" style={{ height: 300 }}>
      ...
      <section className="py-14 sm:py-24 bg-white">
        <div className="container mx-auto px-5 sm:px-6">
          ...
          <div className="text-center mt-10 sm:mt-14">
            <a href="tel:+919000850124" className="inline-flex h-14 items-center justify-center rounded-2xl bg-black px-8 text-sm font-bold uppercase tracking-widest text-white active:scale-[0.97] transition-transform shadow-lg">
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-3 gap-5 sm:gap-8">
            {services.map((s, i) => (
              <div key={i} className={`rounded-3xl border ${s.border} bg-gradient-to-br ${s.accent} p-6 sm:p-8 flex flex-col`}>
                <div className={`rounded-2xl ${s.iconBg} ${s.iconColor} flex items-center justify-center mb-4 sm:mb-6 shadow-sm`} style={{ width: 52, height: 52 }}>
                  {s.icon}
                </div>
                ...
                <a href="tel:+919000850124" className="mt-auto inline-flex items-center gap-2 text-sm font-bold text-gray-900">
                  Enquire now
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
{/* ══ 7.5 MEET THE FOUNDER ════════════════════════════════════════ */}
        ...
        <div className="container mx-auto px-5 sm:px-6">
          <div className="max-w-3xl mx-auto">
            <div className="text-center mb-10 sm:mb-14">
              ...
              <h2 className="text-3xl sm:text-4xl md:text-5xl font-medium text-gray-900 mb-3">FAQ</h2>
      ...
      <section className="py-16 sm:py-28 bg-black relative overflow-hidden">
        ...
        <div className="container mx-auto px-5 sm:px-6 relative z-10 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-stretch sm:items-center justify-center max-w-sm sm:max-w-none mx-auto">
            <a href="tel:+919000850124" className="inline-flex h-14 sm:h-16 items-center justify-center gap-3 rounded-2xl bg-white px-8 sm:px-10 text-sm font-bold uppercase tracking-widest text-black active:scale-[0.97] transition-transform shadow-2xl">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
              </svg>
              Call +91 90008 50124
            </a>
            <a href="https://wa.me/919000850124" target="_blank" rel="noopener noreferrer" className="inline-flex h-14 sm:h-16 items-center justify-center gap-3 rounded-2xl border-2 border-gray-700 px-8 sm:px-10 text-sm font-bold uppercase tracking-widest text-white active:scale-[0.97] transition-transform hover:border-white">
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 z-40 flex gap-0 border-t border-gray-200 bg-white shadow-2xl">
        <a href="tel:+919000850124" className="flex-1 flex flex-col items-center justify-center gap-1 py-3 bg-black text-white active:bg-gray-900 transition-colors">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
          </svg>
          <span className="text-[10px] font-bold uppercase tracking-wide">Call Now</span>
        </a>
        <a href="https://wa.me/919000850124" target="_blank" rel="noopener noreferrer" className="flex-1 flex flex-col items-center justify-center gap-1 py-3 bg-emerald-500 text-white active:bg-emerald-600 transition-colors">
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
          </svg>
